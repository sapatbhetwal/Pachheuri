...
const ProductList = () => {
  const { products, deleteProduct, toggleBestseller, currency, resetCatalogToDefault, isDbLoaded } = useContext(ShopContext)
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs bg-emerald-50 text-emerald-700 font-medium px-2.5 py-1 rounded-full inline-flex items-center gap-1.5 border border-emerald-100">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
              DB: {isDbLoaded ? 'IndexedDB Active' : 'Connecting...'}
            </span>
          </div>
          <p className="text-sm text-gray-400">Manage your persistent product catalog & assets ({products.length} items)</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => setShowResetConfirm(!showResetConfirm)}
            className="text-xs text-gray-500 hover:text-red-600 bg-white border border-gray-200 px-3.5 py-2.5 rounded-full font-medium transition-all"
      ...
      {showResetConfirm && (
        <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between gap-4">
          <div className="text-xs text-amber-900">
            <p className="font-bold">Reset database to default items?</p>
            <p className="text-amber-700">This will reload original 52 catalog items into the database. Custom added products will be replaced.</p>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              onClick={() => setShowResetConfirm(false)}
              className="px-3 py-1 text-xs bg-white text-gray-700 border border-gray-300 rounded-lg"
            >
              Cancel
            </button>
            <button
              ...
              className="px-3 py-1 text-xs bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg"
            >
              Confirm Reset
            </button>
          </div>
